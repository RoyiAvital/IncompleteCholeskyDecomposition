 ...
 *                          Type: 'Double'.
 *                          Range: [0, inf).
 * - relThr				-	Relative Threshold.
 *							Discards element by comparing its absolute value to the L1 norm  
 *							of the elements beneath it to the end of the column. This should
 *							match MATLAB's threshold in `ichol()`.
 *							Structure: Scalar.
 *                          Type: 'Double'.
 *                          Range: [0, inf).
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double *vData, *vDataA, discardThr, relThr, shiftVal;
	...
	{
		mexErrMsgIdAndTxt("IncompleteCholeskyDecomposition:IncompleteCholeskyDecomposition:nrhs", "Relative Threshold: The 3rd input must be a real non negative scalar of type double");
	}
	...
	{
		mexErrMsgIdAndTxt("IncompleteCholeskyDecomposition:IncompleteCholeskyDecomposition:nrhs", "Shift Value: The 4th input must be a real non negative scalar of type double");
	}

	// Validating the 5th input
	if ( !mxIsNumeric(prhs[4]) || mxIsEmpty(prhs[4]) || mxIsComplex(prhs[4]) || (mxGetClassID(prhs[4]) != mxDOUBLE_CLASS) || (mxGetNumberOfElements(prhs[4]) != 1) )
	{
		mexErrMsgIdAndTxt("IncompleteCholeskyDecomposition:IncompleteCholeskyDecomposition:nrhs", "Maximum Number of Non Zeros: The 5th input must be a real positive scalar of type double");
	}
	...
	relThr 		= (double)mxGetScalar(prhs[2]);
	shiftVal	= (double)mxGetScalar(prhs[3]);
	maxNumNz	= (unsigned int)mxGetScalar(prhs[4]);
	...
	icholNnz = _ichol((int) numCols, vDataA, vIndicesA, vIndicesPtrA, vData, vIndices, vIndicesPtr, discardThr, relThr, shiftVal, maxNumNz);
